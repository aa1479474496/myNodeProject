...
var mobileck = {};
mobileck.install = function(Vue, options) {
    // Vue.directive('mck', {
        ...
        bind: function(el, binding, vnode) {
            ...
            $(el).on('click', function() {
                ...
                list.each((index, item) => {
                    ...
                    if (!flag) {
                        errcount++;
                        vm.$set(vm.errorFiled, 'errorText', vm.errorFiled.errorTips[errorName])
                        vm.$set(vm.errorFiled, 'showflag', true)
                        return false
                        // console.log(vm.errorFiled);
                    }
                });
                if (errcount == 0) {
                    vm.checkSumbit();
...
function check(el, errorArr, value, vm) {
    ...
    // flag ? vm.$set(vm.errorFiled, vm.errorFiled.errorText, '') : vm.$set(vm.errorFiled, vm.errorFiled.errorText, vm.errorFiled.errorTips[errorName]);
    return flag;
}
